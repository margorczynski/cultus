    ...
    pub fn next_state(&self, game_action: GameAction) -> GameState {
        match self {
            InProgress(current_level, current_step, current_points) => {
                debug!("next_state for current InProgress - current_step: {}, current_points: {}", *current_step, *current_points);
                ...
                debug!("Level points left: {}, new_points: {}", current_level.get_point_amount(), new_points);
...
mod game_state_tests {
    ...
    fn next_state_too_many_moves_test() {
        ...
        for _ in 0..(max_steps + 1) {
            exhausted_current_game_state = exhausted_current_game_state.next_state(GameAction::MoveUp)
        }
    ...
    fn next_state_win_test() {
        ...
        let won_current_game_state =
            GameState::from_initial_level(test_level.clone())

...
pub fn bit_vector_to_bitstring(bit_vector: &Vec<bool>) -> String {
    bit_vector.iter().map(|&bit| if bit {'1'} else {'0'}).collect()
}
...
mod common_tests {
    ...
    fn bit_vector_to_bitstring_test() {
        setup();
        assert_eq!(bit_vector_to_bitstring(&vec![false, false, true, false, false]), "00100");
        assert_eq!(bit_vector_to_bitstring(&vec![false, false, true, true, true]), "00111");
        assert_eq!(bit_vector_to_bitstring(&vec![false, true, false, true, false, true, false, true, false, true, false, true]), "010101010101");
    }
}